...
public class CompositionsController : TotalArmyBaseController
{
    ...
    public ActionResult<IList<CompositionViewModel>> GetCompositions([FromQuery] string? name,
        [FromQuery] string? battleTypeId, [FromQuery] int? factionId, [FromQuery] int? budget)
    {
        var compositions = _service.GetCompositions(name, battleTypeId, factionId, budget);
    ...
    [HttpGet("{id}/units2/", Name = "GetCompositionUnits2")]
    public ActionResult<IList<UnitViewModel>> GetCompositionUnits2(int id)
    {
        var compositionUnits = _service.GetCompositionUnits2(id);
        return OkOrNoContent(_mapper.Map<IList<UnitViewModel>>(compositionUnits));
    }
